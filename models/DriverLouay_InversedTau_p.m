classdef DriverLouay_InversedTau_p < IdentificationGreyBoxModel
    ...
    properties (Constant)
        ...
        N_output    = 2
        N_param     = 9
        param_name  = {'K_p', 'K_c', 'T_I', 'T_L', 'tau_p_inv', 'K_r', 'K_t', 'T_I', 'v'}
    end
    
    properties (Access = public)
        param_nominal_value = [3.4 15 1 3 25 1 12 0.1 18]
    end
    
    methods
        function obj = DriverLouay_InversedTau_p( Ts, varargin )
        ...
        function [A, B, C, D] = model( obj, param, Ts )
            ...
            [K_p, K_c, T_I, T_L, tau_p_inv, K_r, K_t, T_N, v] = param_cell{:};

            A(1,1)  = -1/T_I;
            A(2,1)  = -(tau_p_inv/2)*(K_c/v)*(T_L/T_I -1);
            A(2,2)  = -(tau_p_inv/2);
            A(3,1)  = 1/T_N*(K_r*v+K_t)*(K_c/v)*(T_L/T_I-1);
            A(3,2)  = 1/T_N*2*(K_r*v+K_t);
            A(3,3)  = -1/T_N;
            B(1,2)  = 1/T_I;
            B(2,1)  = (tau_p_inv/2)*K_p;
            B(2,2)  = (tau_p_inv/2)*(K_c/v)*(T_L/T_I);
            ...
            % A(1,1)  = -1/T_I;
            % A(2,1)  = 2*(tau_p_inv)*(K_c/v)*(T_L/T_I -1);
            % A(2,2)  = -2*(tau_p_inv);
            % A(3,1)  = -1/T_N*(K_r*v+K_t)*(K_c/v)*(T_L/T_I-1);
            % A(3,2)  = 1/T_N*2*(K_r*v+K_t);
            % A(3,3)  = -1/T_N;
            % B(1,2)  = 1/T_I;
            % B(2,1)  = 2*(tau_p_inv)*K_p;            
            % B(2,2)  = -2*(tau_p_inv)*(K_c/v)*(T_L/T_I);
            % B(3,1)  = -1/T_N*(K_r*v+K_t)*K_p;
            % B(3,2)  = 1/T_N*(K_r*v+K_t)*(K_c/v)*(T_L/T_I);
            % B(3,3)  = -1/T_N*K_t;
            % B(3,4)  = -1/T_N;
